...
We use `eepd_fit()` to fit the models, and calculate the prediction errors and BMA weights.
...
We compute the ATT using `eepd_est()`, which uses bootstrapping to compute model uncertainty due to sampling along with uncertainty due to model selection.
